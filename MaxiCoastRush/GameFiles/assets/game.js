...
function buildPatches(seed){const rng=mulberry32(seed^0x9e3779b9);const p=[];for(let c=0;c<9;c++){const base=c*7*STAGE_LEN+STAGE_LEN;for(let i=0;i<6;i++)p.push({d:base+60+rng()*(STAGE_LEN-140),x:(rng()*2-1)*(LANE-0.8),r:2.2+rng()*1.4});}return p;}
function buildWeeds(seed){const rng=mulberry32(seed^0x51ed270b);const w=[];for(let c=0;c<9;c++){const base=c*7*STAGE_LEN+2*STAGE_LEN;for(let i=0;i<5;i++)w.push({d:base+80+rng()*(STAGE_LEN-160),amp:2.0+rng()*2.0,w:0.8+rng()*0.9,ph:rng()*6.283});}return w;}
const weedX=(w,t)=>Math.sin(t*w.w+w.ph)*w.amp;
const biomeAt=d=>Math.floor(Math.max(0,d)/STAGE_LEN)%7;
...
window.onerror=function(msg,src,line,col){
  ...
  e.textContent='ERR: '+msg+' @'+(src||'').split('/').pop()+':'+line+':'+col;
};
/* =================== DOM + strings =================== */
...
{ // image-based lighting: gives metals (the cans!) something to reflect
...
scene.add(new THREE.AmbientLight(0x4477bb,0.62));

/* biome palettes - teal deep / golden hour / bright amber, per the style formula */
const BIO=[
 {bg:0x0f5a86,fog:0x0f5a86,hs:0x5fb6e8,hg:0x14324a,sun:0xbfe6ff,si:1.15,near:28,far:230},
 ...
 {bg:0x9fd8ef,fog:0xd8f0dc,hs:0xdcf4cc,hg:0x4a6a3a,sun:0xfff2d0,si:1.4,near:45,far:250},
 {bg:0x2fd0e4,fog:0x7fd8e4,hs:0xcaf6f2,hg:0x2a7a8a,sun:0xffffff,si:1.45,near:70,far:320}
];
const _c1=new THREE.Color(),_c2=new THREE.Color();
function applyBiome(dist){
  const cyc=dist/STAGE_LEN,idx=Math.floor(cyc)%7,nxt=(idx+1)%7,f=cyc-Math.floor(cyc);
  ...
  scene.fog.near=A.near+(B.near-A.near)*b;scene.fog.far=A.far+(B.far-A.far)*b;
  return {idx,nxt,b};
}
...
function tileTexture(img){
  ...
  t.wrapS=t.wrapT=THREE.MirroredRepeatWrapping;t.anisotropy=4;return t; // seam-proof wrap
}
...
const paintCoral=x=>{x.fillStyle='#d6597a';for(let i=0;i<5;i++){x.beginPath();
  x.ellipse(128+(i-2)*28,200-Math.abs(i-2)*10,16,70+Math.random()*30,0,0,7);x.fill();}
  x.fillStyle='#2f8f5a';for(let i=0;i<4;i++){x.fillRect(70+i*36,120,8,130);}};
...
const WENVS=[];
function buildWorldEnv(gltfScene,biome,target,nearX,alt,byDepth){
  ...
  const slotN=Math.min(3,Math.max(2,Math.ceil(240/len)+1));
  const insts=[];
  ...
  WENVS.push({biome,insts,len,nearX,alt});
}
...
async function loadAssets(){
  ...
  await Promise.allSettled([
    img(ASSETS.tile_seabed).then(i=>TEX.seabed=i?tileTexture(i):fbTile('#1b6e86','#2f9ab0')).catch(()=>TEX.seabed=fbTile('#1b6e86','#2f9ab0')),
    img(ASSETS.tile_beach ).then(i=>TEX.beach =i?tileTexture(i):fbTile('#efe0bd','#d9c391')).catch(()=>TEX.beach =fbTile('#efe0bd','#d9c391')),
    img(ASSETS.tile_desert).then(i=>TEX.desert=i?tileTexture(i):fbTile('#d8a85a','#b9843c')).catch(()=>TEX.desert=fbTile('#d8a85a','#b9843c')),
    img(ASSETS.tile_necro ).then(i=>TEX.necro=i?tileTexture(i):fbTile('#2b2540','#3d3358')).catch(()=>TEX.necro=fbTile('#2b2540','#3d3358')),
    img('./assets/tex/lava.png' ).then(i=>TEX.lavaT=i?tileTexture(i):fbTile('#e85a10','#ffa020')).catch(()=>TEX.lavaT=fbTile('#e85a10','#ffa020')),
    img('./assets/tex/grass.png').then(i=>TEX.grass=i?tileTexture(i):fbTile('#4a7a2a','#5f9a38')).catch(()=>TEX.grass=fbTile('#4a7a2a','#5f9a38')),
    Promise.resolve().then(()=>{TEX.waterT=fbWater();}),
    img(ASSETS.can_label  ).then(i=>{if(i){const t=new THREE.Texture(i);t.needsUpdate=true;t.colorSpace=THREE.SRGBColorSpace;TEX.label=t;}else TEX.label=fbLabel();}).catch(()=>TEX.label=fbLabel()),
    img(ASSETS.spr_coral  ).then(i=>TEX.coral=i?chromaKey(i,0xFF00FF):fbSprite(paintCoral)).catch(()=>TEX.coral=fbSprite(paintCoral)),
    img(ASSETS.spr_coral2 ).then(i=>{if(i)TEX.coral2=chromaKey(i,0xFF00FF);}).catch(()=>{}),
    img(ASSETS.spr_palm   ).then(i=>TEX.palm =i?chromaKey(i,0xFF00FF):fbSprite(paintPalm)).catch(()=>TEX.palm =fbSprite(paintPalm)),
    img(ASSETS.spr_rock   ).then(i=>TEX.rock =i?chromaKey(i,0xFF00FF):fbSprite(paintRock)).catch(()=>TEX.rock =fbSprite(paintRock)),
    (async()=>{try{const g=await new THREE.GLTFLoader().loadAsync('./assets/models/dunes.glb');
      buildEnvPool(g.scene);}catch(e){/* keep rock billboards */}})(),
    (async()=>{try{const g=await new THREE.GLTFLoader().loadAsync('./assets/models/city.glb');
      buildCityPool(g.scene);}catch(e){/* town simply absent */}})(),
    (async()=>{try{const g=await new THREE.GLTFLoader().loadAsync('./assets/models/scatter.glb');
      buildScatterPool(g.scene);}catch(e){/* scatter simply absent */}})(),
    (async()=>{try{const g=await new THREE.GLTFLoader().loadAsync('./assets/models/octopus.glb');
      buildOctoPool(g);}catch(e){/* swimmers simply absent */}})(),
    (async()=>{try{const g=await new THREE.GLTFLoader().loadAsync('./assets/models/cans.glb');
      buildCanProtos(g.scene);}catch(e){/* fallback cylinder can */}})(),
    (async()=>{try{const g=await new THREE.GLTFLoader().loadAsync('./assets/models/cata.glb');
      buildCataPool(g.scene);}catch(e){/* necropolis env simply absent */}})(),
    (async()=>{try{const g=await new THREE.GLTFLoader().loadAsync('./assets/models/fruits.glb');
      buildFruitProtos(g.scene);}catch(e){/* keep procedural pickups */}})(),
    (async()=>{try{const g=await new THREE.GLTFLoader().loadAsync('./assets/models/kawaii.glb');
      buildKawaii(g.scene);}catch(e){/* special simply absent */}})(),
    (async()=>{try{const g=await new THREE.GLTFLoader().loadAsync('./assets/models/zombie.glb');
      buildZombie(g.scene);}catch(e){/* villain simply absent */}})(),
    (async()=>{try{const g=await new THREE.GLTFLoader().loadAsync('./assets/models/lava.glb');
      buildWorldEnv(g.scene,4,80,12,false,true);}catch(e){}})(),
    (async()=>{try{const g=await new THREE.GLTFLoader().loadAsync('./assets/models/village.glb');
      buildWorldEnv(g.scene,5,9,13,false);}catch(e){}})(),
    (async()=>{try{const g=await new THREE.GLTFLoader().loadAsync('./assets/models/water.glb');
      buildWorldEnv(g.scene,6,100,11,false,true);}catch(e){}})()
  ]);
}
...
async function loadAudio(){
  ...
  await Promise.allSettled([load('pickup',ASSETS.sfx_pickup),load('rotten',ASSETS.sfx_rotten),
...
function tuneSurf(biome){ // both players share the room speaker: use P1's biome
  if(surfFilter){const f=biome===0?500:biome===1?4500:biome===3?700:biome===4?420:biome===5?4200:biome===6?3600:2500;
...
function unlockAudio(){if(audioUnlocked)return;audioUnlocked=true;loadAudio().then(()=>{if(phase==='race')startSurf();});}
...
const TILES=()=>[TEX.seabed,TEX.beach,TEX.desert,TEX.necro,TEX.lavaT,TEX.grass,TEX.waterT];
function setGroundTex(mat,idx){const t=TILES()[idx];if(t&&mat.map!==t){mat.map=t;t.repeat.set(4,16);mat.needsUpdate=true;}}
...
function sceneryTexFor(b,slot){if(b===2&&envReady)return null;if(b===3)return cataReady?null:TEX.rock;if(b>=4)return WENVS.some(w=>w.biome===b)?null:(b===5?TEX.palm:TEX.rock);if(b===0)return(slot%2&&TEX.coral2)?TEX.coral2:TEX.coral;return b===1?TEX.palm:TEX.rock;}
...
const FLAVORS=[
  {node:'Orange_Can',   col:'#ff8a2a'},
  {node:'Lemon_Can',    col:'#9fd41e'},
  {node:'Cola_Can',     col:'#3f8fe8'},
  {node:'Lemon_Mint_Can',col:'#cede2a'}
];
...
const FRUIT_SIZE=[1.15,1.0,0.9,0.85];
...
function buildCans(){
  canHolders[0].clear();canHolders[0].add(makeCan(null,P[0].flavor,0));canHolders[0].add(makeTag(STR.p1,'#f6e2ad'));
  canHolders[1].clear();canHolders[1].add(makeCan(0xbfe8ff,P[1].flavor,1));canHolders[1].add(makeTag(STR.p2,'#9fe2ff'));
}
...
/* =================== game state =================== */
function newPlayer(){return{joined:false,flavor:0,stage:0,spCaught:new Set(),zDone:new Set(),z:null,x:0,dist:0,score:0,boost:0,oxy:100,boostT:0,speed:MIN_SPEED,
...
function refreshSlots(){
  for(const i of[0,1]){
    ...
    if(p.stage===0){flv.style.display='none';$('slotSt'+i).textContent=STR.pressJoin;}
    else{
...
function startRace(){
  ...
  for(const p of P){Object.assign(p,{x:0,dist:0,score:0,boost:0,oxy:100,boostT:0,speed:MIN_SPEED,red:0,shake:0});
    p.claimed.clear();p.weedHits.clear();p.spCaught.clear();p.zDone.clear();p.z=null;}
...
function readInput(i){
  ...
  const aEdge=a&&!prevA[i],bEdge=b&&!prevB[i];prevA[i]=a;prevB[i]=b;
  return{steer,rt,aEdge,bEdge};
}
...
function fx(p,i,it){
  if(it.kind===3){p.red=0.85;p.shake=0.9;play('rotten',0.5);addFeed(i,STR.feedRotten,false);}
    ...
    burst(i,p.x,1.3,it.kind===0?0xff9aa0:it.kind===1?0xf6e2ad:0x9dffb0);
...
function layoutWorld(viewer,vi,dt){
  ...
  const wantPath=bio.idx===4?PATH_STONE:bio.idx===5?PATH_COBBLE:null;
  if(wantPath){
    if(pathMat.map!==wantPath){pathMat.map=wantPath;pathMat.needsUpdate=true;}
...
function loop(now){
  ...
  lastT=now;
  const rdt=Math.min(0.05,(now-lastRender)/1000);lastRender=now;
  updateSparks(rdt);
  if(phase==='race'||phase==='count')renderAll(rdt);
  if(phase!=='lobby'&&phase!=='results')updateHud();
  if(dev&&(frames++,now-fpsAt>=500)){$('dev').textContent=Math.round(frames*1000/(now-fpsAt))+' fps';frames=0;fpsAt=now;}
}

/* =================== boot =================== */
resize();
enterLobby();
loadAssets().then(()=>{if(phase!=='lobby')buildCans();});
requestAnimationFrame(loop);
